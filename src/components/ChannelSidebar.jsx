...
const ChannelSidebar = () => {
  ...
  const mappedChannels = channels.map((channel) => (
    <div onClick={() => {setCurrentChannel(channel.name)
      fetchPaginatedMessages(selectedChatroom, selectedChannel.name, 1, 10)
    }} key={channel._id} className="group flex items-center gap-2.5 w-9/10 h-8 pl-1 rounded-md hover:bg-zinc-700">
